...
const GeoButtons = (GOOGLE_GEOCODING_API_KEY) => {
  const getGeoLocation = () => {
    const googleData = fetch(
      `https://www.googleapis.com/geolocation/v1/geolocate?key=${GOOGLE_GEOCODING_API_KEY}`
    );
  ...
  const getTimeZone = (long, lat) => {
    const timeZoneData = fetch(
      ...
      `https://maps.googleapis.com/maps/api/timezone/json?location=51.5135808,0.0244542&timestamp=1331766000&key=${GOOGLE_GEOCODING_API_KEY}`
    );
